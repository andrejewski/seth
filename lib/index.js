...
Set.prototype.setMembers = function(members) {
	...
	if(Array.isArray(members)) {
		...
		members = _bind(_contains, null, members);
	}
...
Proof.prototype.confirmsAll = function(xs) {
	var F = this;
	return _reduce(xs, function(s, x) {
...
Proof.prototype.testArray = function(xs) {
	return _map(xs, _bind(this.confirms, this));
}
...
function _forEach(array, fx) {
	if(Array.prototype.map) return array.forEach(fx);
	for(var i in array) {
		fx(array[i], parseInt(i, 10));
	}
}

function _map(array, fx) {
	if(Array.prototype.map) return array.map(fx);
	var results = [];
	_forEach(array, function(item) {
		results.push(fx(item));
	});
	return results;
}

function _reduce(array, fx, i) {
	if(Array.prototype.reduce) return array.reduce(fx, i);
	if(array.length === 0) return i;
	var memo = typeof i !== 'undefined' ? i : array.shift();
	_forEach(array, function(item) {
		memo = fx(memo, item, array);
	});
	return memo;
}
...
function _bind(fx) {
	...
	return Function.prototype.bind
		? Function.prototype.bind.apply(fx, args)
		: (function() {
			var _this = args.shift();
			return function() {
				var params = Array.prototype.slice.call(arguments, 0);
				return fx.apply(_this, args.concat(params));
			};
		})();
}
